...
public class IabHelperImpl {
        ...
        this.mServiceConn = new ServiceConnection() {
            ...
            public void onServiceConnected(ComponentName name, IBinder service) {
                if (!IabHelperImpl.this.mDisposed) {
                    ...
                    IabHelperImpl.this.mService = Stub.asInterface(service);
                    ...
                    try {
                        ...
                        if (IabHelperImpl.this.mSubscriptionUpdateSupported) {
                            ...
                            response = IabHelperImpl.this.mService.isBillingSupported(3, packageName, IabHelperImpl.ITEM_TYPE_SUBS);
                            if (response == 0) {
                                IabHelperImpl.this.logDebug("Subscriptions AVAILABLE.");
                                IabHelperImpl.this.mSubscriptionsSupported = true;
                            } else {
                                IabHelperImpl.this.logDebug("Subscriptions NOT AVAILABLE. Response: " + response);
                                IabHelperImpl.this.mSubscriptionsSupported = false;
                                IabHelperImpl.this.mSubscriptionUpdateSupported = false;
                            }
                        }
    ...
    public Inventory queryInventory(boolean querySkuDetails, List<String> moreItemSkus, List<String> list) throws IabException {
        if (this.mService == null) {
            throw new IabException((int) IABHELPER_SERVICE_UNAVAILABLE, "Billing service is null");
        }
        checkNotDisposed();
    ...
    public void queryInventoryAsync(boolean querySkuDetails, List<String> moreItemSkus, List<String> moreSubsSkus, QueryInventoryFinishedListener listener) throws IabAsyncInProgressException {
        ...
        new Thread(new Runnable() {
            public void run() {
                ...
                Inventory inv = null;
                try {
                    inv = IabHelperImpl.this.queryInventory(z, list, list2);
                ...
                if (!IabHelperImpl.this.mDisposed && queryInventoryFinishedListener != null) {
    ...
    int queryPurchases(Inventory inv, String itemType) throws JSONException, RemoteException {
        ...
        do {
            ...
            if (response != 0) {
                ...
                for (int i2 = 0; i2 < purchaseDataList.size(); i2++) {
    ...
    void consumeAsyncInternal(List<Purchase> purchases, OnConsumeFinishedListener singleListener, OnConsumeMultiFinishedListener multiListener) throws IabAsyncInProgressException {
        ...
        new Thread(new Runnable() {
            public void run() {
                ...
                IabHelperImpl.this.flagEndAsync();
                if (!(IabHelperImpl.this.mDisposed || onConsumeFinishedListener == null)) {
                    Handler handler = handler;
                ...
                if (!IabHelperImpl.this.mDisposed && onConsumeMultiFinishedListener != null) {
                    handler = handler;
