...
public class ViewPager extends ViewGroup {
    ...
    private static final Interpolator sInterpolator = new Interpolator() {
        public float getInterpolation(float t) {
            t -= TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER;
            return ((((t * t) * t) * t) * t) + TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER;
        }
    };
    ...
    void smoothScrollTo(int x, int y, int velocity) {
        ...
        float distance = ((float) halfWidth) + (((float) halfWidth) * distanceInfluenceForSnapDuration(Math.min(TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER, (TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER * ((float) Math.abs(dx))) / ((float) width))));
        velocity = Math.abs(velocity);
        if (velocity > 0) {
            duration = Math.round(1000.0f * Math.abs(distance / ((float) velocity))) * 4;
        } else {
            duration = (int) ((TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER + (((float) Math.abs(dx)) / (((float) this.mPageMargin) + (((float) width) * this.mAdapter.getPageWidth(this.mCurItem))))) * 100.0f);
        }
    ...
    private void calculatePageOffsets(ItemInfo curItem, int curIndex, ItemInfo oldCurInfo) {
        ...
        this.mLastOffset = curItem.position == N + -1 ? (curItem.offset + curItem.widthFactor) - TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER : AutoScrollHelper.NO_MAX;
        int i = curIndex - 1;
        while (i >= 0) {
        ...
        while (i < itemCount) {
            ...
            if (ii.position == N - 1) {
                this.mLastOffset = (ii.widthFactor + offset) - TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER;
            }
            ii.offset = offset;
            offset += ii.widthFactor + marginOffset;
            i++;
    ...
    public void draw(Canvas canvas) {
        ...
        if (overScrollMode == 0 || (overScrollMode == 1 && this.mAdapter != null && this.mAdapter.getCount() > 1)) {
            ...
            if (!this.mRightEdge.isFinished()) {
                ...
                height = (getHeight() - getPaddingTop()) - getPaddingBottom();
                canvas.rotate(90.0f);
                canvas.translate((float) (-getPaddingTop()), (-(this.mLastOffset + TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER)) * ((float) width));
