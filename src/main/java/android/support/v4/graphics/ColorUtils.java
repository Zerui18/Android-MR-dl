...
public final class ColorUtils {
    ...
    public static void RGBToHSL(@IntRange(from = 0, to = 255) int r, @IntRange(from = 0, to = 255) int g, @IntRange(from = 0, to = 255) int b, @NonNull float[] outHsl) {
        ...
        if (max == min) {
            ...
            s = deltaMaxMin / (TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER - Math.abs((2.0f * l) - TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER));
        }
        h = (60.0f * h) % 360.0f;
        if (h < 0.0f) {
            h += 360.0f;
        ...
        outHsl[1] = constrain(s, 0.0f, (float) TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER);
        outHsl[2] = constrain(l, 0.0f, (float) TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER);
    }
    ...
    public static int HSLToColor(@NonNull float[] hsl) {
        ...
        float s = hsl[1];
        float l = hsl[2];
        float c = (TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER - Math.abs((2.0f * l) - TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER)) * s;
        float m = l - (0.5f * c);
        float x = c * (TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER - Math.abs(((h / 60.0f) % 2.0f) - TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER));
        int r = 0;
        int g = 0;
        int b = 0;
        switch (((int) h) / 60) {
    ...
    public static int blendARGB(@ColorInt int color1, @ColorInt int color2, @FloatRange(from = 0.0d, to = 1.0d) float ratio) {
        float inverseRatio = TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER - ratio;
