...
public abstract class SharedElementCallback {
    ...
    public Parcelable onCaptureSharedElementSnapshot(View sharedElement, Matrix viewToGlobalMatrix, RectF screenBounds) {
        ...
        if (bitmapWidth > 0 && bitmapHeight > 0) {
            float scale = Math.min(TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER, ((float) MAX_IMAGE_SIZE) / ((float) (bitmapWidth * bitmapHeight)));
    ...
    private static Bitmap createDrawableBitmap(Drawable drawable) {
        ...
        float scale = Math.min(TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER, ((float) MAX_IMAGE_SIZE) / ((float) (width * height)));
        if ((drawable instanceof BitmapDrawable) && scale == TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER) {
