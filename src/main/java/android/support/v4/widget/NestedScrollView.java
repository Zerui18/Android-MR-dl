                        ...
                        if (!dispatchNestedScroll(0, scrolledDeltaY, 0, deltaY - scrolledDeltaY, this.mScrollOffset)) {
                            if (canOverscroll) {
                                ...
                                if (pulledToY < 0) {
                                    ...
                                    this.mEdgeGlowBottom.onPull(((float) deltaY) / ((float) getHeight()), TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER - (MotionEventCompat.getX(ev, activePointerIndex) / ((float) getWidth())));
