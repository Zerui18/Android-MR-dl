...
public class ViewDragHelper {
    ...
    public static ViewDragHelper create(ViewGroup forParent, float sensitivity, Callback cb) {
        ...
        helper.mTouchSlop = (int) (((float) helper.mTouchSlop) * (TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER / sensitivity));
        return helper;
    }
    ...
    private int computeAxisDuration(int delta, int velocity, int motionRange) {
        ...
        float distance = ((float) halfWidth) + (((float) halfWidth) * distanceInfluenceForSnapDuration(Math.min(TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER, ((float) Math.abs(delta)) / ((float) width))));
        velocity = Math.abs(velocity);
        if (velocity > 0) {
            duration = Math.round(1000.0f * Math.abs(distance / ((float) velocity))) * 4;
        } else {
            duration = (int) (((((float) Math.abs(delta)) / ((float) motionRange)) + TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER) * 256.0f);
        }
