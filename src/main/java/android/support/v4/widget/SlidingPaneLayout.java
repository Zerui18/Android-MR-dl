...
public class SlidingPaneLayout extends ViewGroup {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (this.mFirstLayout) {
            float f = (this.mCanSlide && this.mPreservedOpenState) ? TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER : 0.0f;
            this.mSlideOffset = f;
        }
        for (i = 0; i < childCount; i++) {
            View child = getChildAt(i);
            if (child.getVisibility() != 8) {
                ...
                if (lp.slideable) {
                    ...
                    offset = (int) ((TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER - this.mSlideOffset) * ((float) this.mParallaxBy));
                    xStart = nextXStart;
                }
                if (isLayoutRtl) {
    ...
    private boolean openPane(View pane, int initialVelocity) {
        if (!this.mFirstLayout && !smoothSlideTo(TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER, initialVelocity)) {
            return false;
        }
    ...
    public boolean isOpen() {
        return !this.mCanSlide || this.mSlideOffset == TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER;
    }
    ...
    private void parallaxOtherViews(float slideOffset) {
            ...
            if ((isLayoutRtl ? slideLp.rightMargin : slideLp.leftMargin) <= 0) {
                ...
                for (i = 0; i < childCount; i++) {
                    ...
                    if (v == this.mSlideableView) {
                        int oldOffset = (int) ((TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER - this.mParallaxOffset) * ((float) this.mParallaxBy));
                        this.mParallaxOffset = slideOffset;
                        dx = oldOffset - ((int) ((TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER - slideOffset) * ((float) this.mParallaxBy)));
                        if (isLayoutRtl) {
                            dx = -dx;
                        }
                        v.offsetLeftAndRight(dx);
                        if (!dimViews) {
                            dimChildView(v, isLayoutRtl ? this.mParallaxOffset - TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER : TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER - this.mParallaxOffset, this.mCoveredFadeColor);
                        }
                    }
                }
            }
        ...
        for (i = 0; i < childCount; i++) {
            v = getChildAt(i);
            if (v == this.mSlideableView) {
                int oldOffset2 = (int) ((TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER - this.mParallaxOffset) * ((float) this.mParallaxBy));
                this.mParallaxOffset = slideOffset;
                dx = oldOffset2 - ((int) ((TapjoyConnectCore.DEFAULT_CURRENCY_MULTIPLIER - slideOffset) * ((float) this.mParallaxBy)));
                if (isLayoutRtl) {
                    dx = -dx;
                }
                v.offsetLeftAndRight(dx);
